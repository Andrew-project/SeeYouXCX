...
function init(page, opt) {
    ...
    maxVoiceTime =
        opt.maxVoiceTime && opt.maxVoiceTime <= 60 ? opt.maxVoiceTime : 60;
    voiceFormat = opt.format || "mp3";
    startTimeDown =
        opt.startTimeDown &&
        opt.startTimeDown < maxVoiceTime &&
        opt.startTimeDown > 0 ?
        opt.startTimeDown :
        54;
        ...
        typeof cbOk === "function" &&
            recorderManager.onStop(function(res) {
                ...
                if (_page.data.inputObj.voiceObj.status === "short") {
                    ...
                    typeof startVoiceRecordCbOk === "function" &&
                        startVoiceRecordCbOk(status.SHORT);
                    return;
                } else if (_page.data.inputObj.voiceObj.moveToCancel) {
                    typeof startVoiceRecordCbOk === "function" &&
                        startVoiceRecordCbOk(status.CANCEL);
                    return;
                }
                console.log("录音成功");
                typeof startVoiceRecordCbOk === "function" &&
                    startVoiceRecordCbOk(status.SUCCESS);
                typeof sendVoiceCbOk === "function" &&
                    sendVoiceCbOk(res, Math.round(res.duration / 1000));
            });
        typeof cbError === "function" &&
            recorderManager.onError(function(res) {
                typeof startVoiceRecordCbOk === "function" &&
                    startVoiceRecordCbOk(status.FAIL);
...
function initExtraData(extra$arr) {
    ...
    _page.chatInputExtraClickEvent = function() {
        _page.setData({
            "inputObj.extraObj.chatInputShowExtra": !_page.data.inputObj.extraObj
                .chatInputShowExtra
        });
        extraButtonClickEvent &&
            extraButtonClickEvent(!_page.data.inputObj.extraObj.chatInputShowExtra);
    };
}
...
function dealVoiceLongClickEventWithHighVersion() {
    recorderManager.onStart(function() {
        ...
        timer = setInterval(function() {
            ...
            if (
                singleVoiceTimeCount >= startTimeDown &&
                singleVoiceTimeCount < maxVoiceTime
            ) {
                _page.setData({
    ...
    _page.longClickVoiceBtn = function(e) {
        if ("send$voice$btn" === e.detail.id) {
            //长按时需要打开录音功能，开始录音
            _page.setData({
                //调出取消弹窗
            ...
            typeof startVoiceRecordCbOk === "function" &&
                startVoiceRecordCbOk(status.START);
            checkRecordAuth(
                function() {
                ...
                function(res) {
                    ...
                    typeof startVoiceRecordCbOk === "function" &&
                        startVoiceRecordCbOk(status.UNAUTH);
                    ...
                    wx.startRecord({
                        success: function(res) {
                            ...
                            typeof startVoiceRecordCbOk === "function" &&
                                startVoiceRecordCbOk(status.SUCCESS);
                            typeof sendVoiceCbOk === "function" &&
                                sendVoiceCbOk(res, singleVoiceTimeCount + "");
                        },
                        fail: res => {
                            typeof startVoiceRecordCbOk === "function" &&
                                startVoiceRecordCbOk(status.FAIL);
...
function dealVoiceMoveEvent() {
    _page.sendVoiceMoveEvent = function(e) {
        if ("send$voice$btn" === e.detail.id) {
...
function dealVoiceMoveEndEvent() {
    _page.sendVoiceMoveEndEvent = function(e) {
        if ("send$voice$btn" === e.detail.id) {
